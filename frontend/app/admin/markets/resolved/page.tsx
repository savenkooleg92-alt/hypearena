...
export default function AdminResolvedPage() {
  ...
  const handleDeleteSelected = async () => {
    ...
    if (deleteMode === null) {
      alert('Choose an option: remove without refunds, or remove and return stakes to bettors.');
      return;
    }
    const withRefund = deleteMode === 'refund';
    if (!confirm(withRefund
      ? `Remove ${selectedIds.size} market(s) and return all stakes to bettors? (Winners will have their profit deducted, everyone gets their stake back.)`
  ...
  return (
    <div>
      <h1 className="text-2xl font-bold text-gray-900 dark:text-white mb-2">Resolved battles</h1>
      <p className="text-sm text-gray-500 dark:text-gray-400 mb-4">
        All battles with status RESOLVED. When deleting, choose: just remove from site, or remove and return all stakes to bettors.
      </p>
      <div className="bg-white dark:bg-dark-card rounded-lg shadow overflow-hidden border border-transparent dark:border-[rgba(255,255,255,0.08)]">
        {markets.length === 0 ? (
          <p className="p-6 text-gray-500 dark:text-gray-400">No resolved battles.</p>
        ) : (
          <>
                ...
                <li key={m.id} className="px-4 py-4 flex flex-wrap items-center gap-3">
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <p className="text-xs text-gray-500 dark:text-gray-400 mt-0.5">
                      {m.outcomes?.join(' / ') ?? '—'} · {m._count.bets} bets · {m.category ?? '—'}
                    </p>
                  </div>
                </li>
              ))}
